...
/* degree sin */
static double dsin(double d) { return sin(deg2rad(d)); }

/* degree cos */
static double dcos(double d) { return cos(deg2rad(d)); }

/* degree tan */
static double dtan(double d) { return tan(deg2rad(d)); }

/* degree arcsin */
static double darcsin(double x) { return rad2deg(asin(x)); }

/* degree arccos */
static double darccos(double x) { return rad2deg(acos(x)); }

/* degree arctan */
static double darctan(double x) { return rad2deg(atan(x)); }

/* degree arctan2 */
static double darctan2(double y, double x) { return rad2deg(atan2(y, x)); }

/* degree arccot */
static double darccot(double x) { return rad2deg(atan(1.0 / x)); }
...
void set_asr_method(JuristicMethod method_id) { asr_juristic = method_id; }
...
double sun_declination(double jd) { return sun_position(jd).first; }

/* compute equation of time */
double equation_of_time(double jd) { return sun_position(jd).second; }
...
double compute_time(double g, double t) {
  ...
  double v = 1.0 / 15.0 * darccos((-dsin(g) - dsin(d) * dsin(latitude)) /
...
void compute_times(double times[]) {
  ...
  times[Subuh] =
      compute_time(180.0 - method_params[calc_method].fajr_angle, times[Subuh]);
  ...
  times[Maghrib] =
      compute_time(method_params[calc_method].maghrib_value, times[Maghrib]);
  times[Isya] =
      compute_time(method_params[calc_method].isha_value, times[Isya]);
}
...
static String int_to_string(int num) { return String(num); }
...
void adjust_high_lat_times(double times[]) {
  double night_time =
      time_diff(times[Sunset], times[Sunrise]); // sunset to sunrise

  // Adjust Fajr
  double fajr_diff =
      night_portion(method_params[calc_method].fajr_angle) * night_time;
  if (isnan(times[Subuh]) ||
      time_diff(times[Subuh], times[Sunrise]) > fajr_diff)
    times[Subuh] = times[Sunrise] - fajr_diff;

  // Adjust Isha
  double isha_angle = method_params[calc_method].isha_is_minutes
                          ? 18.0
...
void adjust_times(double times[]) {
  ...
  if (method_params[calc_method].maghrib_is_minutes) // Maghrib
    times[Maghrib] =
        times[Sunset] + method_params[calc_method].maghrib_value / 60.0;
...
void get_prayer_times(int year, int month, int day, double _latitude,
                      double _longitude, double _timezone, double times[]) {
  latitude = _latitude;
  longitude = _longitude;
  timezone = _timezone;
  julian_date =
      get_julian_date(year, month, day) - longitude / (double)(15 * 24);
  compute_day_times(times);
}

void set_calc_method(CalculationMethod method_id) { calc_method = method_id; }
...
void set_dhuhr_minutes(double minutes) { dhuhr_minutes = minutes; }
